...
y=f(x,k)
\end{equation}
with \(\text{dim}(y)=1\times m\), \(\text{dim}(x)=1\times n\), and \(\text{dim}(k)=1\times p\), a single experiment consists of the measurement of each of the \(m\) response variables for a given set of values of the \(n\) independent variables. For each experiment, the measured output vector which can be viewed as a random variable is comprised of the deterministic part calculated from the model and the stochastic part represented by the error term
\begin{equation}
\hat{y}_i=f(x_i,k)+\epsilon_i;~~i=1,2,\cdots,N
\end{equation}
The unkown parameters are found by minimizing a scalar function referred to as the objective function which is a suitable measure of the overall departure of the model calculated values from the measurements. Residual is departure of measurement from the model calcuated value
\begin{equation}
e=\hat{y}-f(x,k)
\end{equation}
The error term \(\epsilon_i\) corresponds to the true parameter values, whereas the residual term \(e_i\) corresponds to the estimated parameter values.
*** Explicit Estimation
The parameters can be obtained by minimizing the least squares (LS) objective function as the weighted sum of squares of the residuals with \(N\) measurements
\begin{equation}
S_{LS}(k)=\sum_{i=1}^N e_i^\top Q_ie_i=\sum_{i=1}^N[\hat{y}_i-f(x_i,k)]^\top Q_i[\hat{y}_i-f(x_i,k)]
\end{equation}
where \(Q_i\) is an \(m\times m\) user-supplied weighting matrix.
**** Simple Least Squares (LS) Estimation
In this case, we minimize the sum of squares of errors (SSE) with \(Q=I\).
**** Weighted Least Squares (WLS) Estimation
In this case, we minimize the SSE with constant weights, \(Q_i=Q\).
**** Generalized Least Squares (GLS) Estimation
In this case, we minimize the SSE with non-constant weights. Practical guidelines for the selection of \(Q\) can be derived from maximum likelihood (ML) considerations.
**** Maximum Likelihood (ML) Estimation
It is reasonable to assume that the measured responses from the \(i^{th}\) experiment are normally ditrubuted. The joint probability density function conditional on the value of the parameters is
\begin{equation} 
\(p(\hat{y}_i|k,\Sigma_i)\sim \det(\Sigma_i)^{-1/2}\exp\left\{-\frac{e_i^\top\Sigma_i^{-1}e_i}{2}\right\}
\end{equation}
where \(\Sigma_i\) is the covariance matrix of the response variables \(y\) at the \(i^{th}\) experiment and hence, of the residuals \(e_i\) too.
If different experiments are assumed independent, the joint probability density function for the all the measured responses is simply the product
\begin{equation}
p(\hat{y}_1,\hat{y}_2,\cdots,\hat{y})N|k,\Sigma_1,\Sigma_2,\cdots,\Sigma_N)\sim\left(\prod_{i=1}^N\det(\Sigma_i)\right)^{-1/2}\exp\left(-\frac{1}{2}\sum_{i=1}^N e_i^\top\Sigma_i^{-1}e_i\right)
\end{equation}
The Loglikelihood is the log of the joint probability function. With known \(\Sigma_i\), the objective funtion for the ML parameter estimates is
\begin{equation}
S_{ML}(k)=\sum_{i=1}^N e_i^\top \Sigma_i^{-1}e_i
\end{equation}
Therefore, on statistical grounds
\begin{equation}
Q_i=\Sigma_i^{-1}
\end{equation}
Since the exact knowledge of all covariance matrices is often impossible to gain, reasonable assumptions about the structure of \(\Sigma_i\) is necessary.
Case 1: Error terms in each response variable and for experiment are all identically and independently distrubuted (i.i.d)
\begin{equation}
\Sigma_i=\sigma_e^2I
\end{equation}
Case 2: Different response varaibles have different variances but constant from experiment to experiment.
\begin{equation}
\Sigma_i=
\[
\begin{bmatrix}
\sigma_{e1}^2 & 0 & \cdots & 0\\
0 & \sigma_{e2}^2 & \cdots & 0\\
\vdots & \vdots & \ddots & \vdots\\
0 & 0 & \cdots & \sigma_{em}^2
\end{bmatrix}
\]
\end{equation}
Case 3: Generalized LS estimation will yield ML estimates whenever the errors are distributed with variances that change from experiment to experiment. 
\begin{equation}
Q_i=\text{COV}(\epsilon_i)^{-1}
\end{equation}
For the case when the error is proportional to each variable,
\begin{equation}
\sigma_{e_{ij}}^2=\sigma^2y_{ij}^2;~~i=1,2,\cdots,N,~j=1,2,\cdots,m
\end{equation}
the ML parameter estimates are the same as the GLS estimates when the weighting martix is
\begin{equation}
Q_i=\text{diag}(y_{i1}^{-2},y_{i2}^{-2},\cdots,y_{im}^{-2});~~i=1,2,\cdots,N
\end{equation}
Since that the weights are a function of the unknown parameters, the measurements can be used if the error is not excessive.
\begin{equation}
Q_i=\text{diag}(\hat{y}_{i1}^{-2},\hat{y}_{i2}^{-2},\cdots,\hat{y}_{im}^{-2});~~i=1,2,\cdots,N
\end{equation}
...
*** Gauss-Newton method for ODE
The model is given by the ODEs
\begin{equation}
\frac{dy}{dt}=f(y,k)
\end{equation}
The objective funtion is of the form
\begin{equation}
S(k)=\sum_{i=1}^N[\hat{y}_i-y(t_i,k)]^\top Q_i[\hat{y}_i-y(t_i,k)]
\end{equation}
Linearization of the output vector around \(k^{(j)}\) and retaining first order terms yields
\begin{equation}
y(t_i,k^{(j+1)})=y(t_i,k^{(j)})+J(t_i)\Delta k^{(j+1)}
\end{equation}
where \(J=\partial y/\partial k\).
In the case of ODE models, the Jacobian matrix cannot be obtained by a simple differentiation. Sensitivity differential equation for Jacobian matrix should be obtained.
Differentiate the model ODEs
\begin{equation}
\frac{\partial}{\partial k}\left(\frac{dy}{dt}\right)=\frac{\partial}{\partial k}\left(f(y,k)\right)
\end{equation}
By reversing the order of differenciation on the left side and performing the implicit differenciation on the right side,
\begin{equation}
\frac{d}{dt}\left(\frac{\partial y}{\partial k}\right)=\frac{\partial f}{\partial y}\frac{\partial y}{\partial k}+\frac{\partial f}{\partial k}\frac{\partial k}{\partial k}
\end{equation}
so that
\begin{equation}
\frac{dJ}{dt}=J\frac{\partial f}{\partial y}+\frac{\partial f}{\partial k}
\end{equation}
and let \(J(t_0)=0\).
This represent a set of \(n\times p\) ODEs.
Substitition of \(y(t_i,k^{(j+1)})\) into the objective function yields
\begin{align*}
S(k^{(j+1)})&=\sum_{i=1}^N[\hat{y}_i-y(t_i,k^{(j+1)})]^\top Q_i[\hat{y}_i-y(t_i,k^{(j+1)})]\\
            &=\sum_{i=1}^N[\hat{y}_i-y(t_i,k^{(j)})-J(t_i)\Delta k^{(j+1)}]^\top Q_i[\hat{y}_i-y(t_i,k^{(j)})-J(t_i)\Delta k^{(j+1)}]
\end{align*}
Use of the stationary condition
\begin{equation*}
\frac{\partial S(k^{(j+1)})}{\partial k^{(j+1)}}&=2\sum_{i=1}^N-J(t_i)Q_i[\hat{y}_i-y(t_i,k^{(j)})-J(t_i)\Delta k^{(j+1)}]=0
\end{equation*}
Therefore,
\begin{equation}
\sum_{i=1}^N J(t_i)Q_iJ(t_i)\Delta k^{(j+1)}=\sum_{i=1}^N J(t_i)Q(i)[\hat{y}_i-y(t_i,k^{(j)})]
\end{equation}
Solving the linear equation yields \(\Delta k^{(j+1)}\) and
\begin{equation}
k^{(j+1)}=k^{(j)}+\mu\Delta k^{(j+1)}
\end{equation}
where \(\mu\) is a stepping parameter \((0<\mu\leq 1)\).

*** Algorithm of GN for ODE models
1. Input the initial guess for the parameters and NSIG
2. Repeat until
3. Solve the ODEs for y and J. At each sampling period, \(t_i,~i=1,\cdots,N\), compute \(y(t_i,k^{(j)})\) and \(J(t_i)\) to set up matrix A=JQJ and vector \(b=JQ[\hat{y}-y]\).
4. Solve the equation \(A\Delta k^{(j+1)}=b\)
5. Determine \(\mu\) using the bisection rule and obtain \(k^{(j+1)}=k^{(j)}+\mu\Delta k^{(j+1)}\)
6. Continue until \(\frac{1}{p}\sum_{i=1}^p\left\lvert\frac{\Delta k_i^{(j+1)}}{k_i^{(j)}}}\right\rvert\leq10^{-NSIG}\)

*** Implementation
**** ODEs
The Jacobian matrix is
\begin{equation}
J(t)=&\frac{\partial y}{\partial k}=\left[\frac{\partial y}{\partial k_1},\cdots,\frac{\partial y}{\partial k_p}\right]=[J_1,\cdots,J_p]
\end{equation}
Each column satisfies the following ODEs
\begin{equation}
\frac{dJ_i(t)}{dt}=\frac{\partial f}{\partial y}J_i(t)+\frac{\partial f}{\partial k_i},~~J_i(t_0)=0\text{   for }i=1,\cdots,p
\end{equation}
Generate \(n\times(p+1)\) dimensional matrix z
\begin{equation}
z=\begin{bmatrix}
y\\
J_1\\
...
f\\
\frac{\partial f}{\partial y}J_1+\frac{\partial f}{\partial k_1}\\
\vdots\\
\frac{\partial f}{\partial y}J_p+\frac{\partial f}{\partial k_p}
\end{bmatrix}
\end{equation}
**** Bisection rule
For the stepping parameter \(\mu\), start with \(\mu=1\) and halving \(\mu\) until
\begin{equation}
S(k^{(j)}+\mu\Delta k^{(k+1)})<S(k^{(j)})
\end{equation}
*** Code for parameter estimation
    #+begin_src python :session pmb :exports code
      def gn_ode(
    #+end_src

* Test of parameter estimation for ODEs
A homogeneous gas phase reaction
\begin{equation*}
2NO+O_2\longleftrightarrow2NO_2
\end{equation*}
with the model
\begin{equation*}
\frac{d[NO_2]}{dt}=k_1(126.2-[NO_2])(91.9-[NO_2])^2-k_2[NO_2]^2;~~[NO_2]_0=0
\end{equation*}
and the measuremets are
